    ...
    <td valign="top" bgcolor="#FFFFFF"> 
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr> 
          ...
          <td width="95%" height="35" align="left" background="images/titlebg.gif"><strong>果类信息编辑</strong></td> 
        </tr>
      </table>
      <?php
	  $ID=$_GET['ID'];
	  ...
	  $sql="Select * from FruitType where ID=".$ID."";
	  $result=$db->db_query($sql);
	  $rs=mssql_fetch_array($result);
	  ?>
	  <div class="RightBody">
      <form method="post" id="Form" name="Form" action="FruType_Save.php?Action=Edit&ID=<?=$rs['ID']?>" onsubmit="return CheckForm();">
      <table class="TableInfo">
        <tr>
		 <td width="80" align="right">归类编号：</td>
		 <td><input name="FruitTypeNo" type="text" value="<?=$rs['FruitTypeNo']?>" readonly="readonly"/></td>
	    </tr>
		<tr>
		 <td width="80" align="right">果类名称：</td>
		 <td><input name="FruitTypeName" type="text" value="<?=$rs['FruitTypeName']?>"/>
