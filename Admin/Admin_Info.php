...
<body>
<form name="Form" id="Form" method="post" onsubmit="return DelAllInfo()" action="Admin_Save.php?Action=DelAll">
    ...
    <td valign="top" bgcolor="#FFFFFF">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ToptitleInfo">
        <tr> 
          ...
          <td width="80%" height="35" align="left" background="images/titlebg.gif"><strong>管理员信息</strong></td>
		  <td width="8%" height="35" align="center" valign="middle" background="images/titlebg.gif"><img src="images/syetem_management.png" width="14" height="14" /> <a href="Admin_Add.php">增加</a></td>
      ...
      <table class="TableInfo" onmouseover="changeto()" onmouseout="changeback()">
		...
		<?php
		 $sqlA="Select * from AdminInfo";
		 $resultA=$db->db_query($sqlA);
		 $total=mssql_num_rows($resultA);
		 $Page=new page_link($total,10);
		 
		$sql="Select * from(Select row_number() over(order by ID asc) as RowNum ,* from AdminInfo) as TableInfo where RowNum between ".$Page->firstcount." and ".$Page->displaypg."";
		...
		<tr class="ListContent">
		 ...
		 ?>
            ...
            <a href="Admin_Save.php?Action=Disable&amp;ID=<?=$rs['ID']?>">启用</a>
         <?php
		 }else{
		 ?>
         	<img src="images/error.gif" width="16" height="16"/>
            <a href="Admin_Save.php?Action=Enable&amp;ID=<?=$rs['ID']?>">禁用</a>
         <?php
		 }
		 ?>
         </td>
		 ...
		 <td class="Operation">
            ...
            <a href="Admin_Edit.php?ID=<?=$rs['ID']?>">编辑</a>&nbsp; &nbsp;
			<img src="images/del.gif"  width="16" height="16" />
            <a href="Admin_Save.php?Action=Del&amp;ID=<?=$rs['ID']?>" onclick="return DelInfo()">删除</a>
		 </td>
	    </tr>
        <?php
		  }
